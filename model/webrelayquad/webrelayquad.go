...
func probe(c *webrelay.Client) (bool, webrelay.Model) {
	found, err := IsWebRelayQuad(c); 
	if err != nil || !found {
		return false, nil
	}
	return true, WebRelayQuadModel(c)
}
...
func IsWebRelayQuad(c *webrelay.Client) (bool, error) {
	url := &url.URL{
		Scheme: "http",
		Host:   c.Host,
		Path:   "menu.html",
...
func WebRelayQuadModel(c *webrelay.Client) webrelay.Model {
	return &webRelayQuad{
		c: c,
	}
}
